...
function getUserPosts() {
    ...
    postsXmlHttp.onreadystatechange = function() {
        if(postsXmlHttp.readyState == 4)
            if(postsXmlHttp.status == 200) {
                ...
                for(var idx=0; idx<postListObj.length; idx++) {
                    ...
                    '<img src="' + base_url + 'media/imagePosts/' + post.id + '.jpg" height="230px" width="350px">'
                }
            }
    };
...
//funzione per l'attivazione delle notifiche di un external profile
function notifica(){
